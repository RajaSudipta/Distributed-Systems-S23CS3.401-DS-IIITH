...
vector<pair<int,int>> merge_subarray(vector<pair<int,int>>&key_freq,int elements_per_proc,int size_Of_Cluster){
    ...
    vector<priority_queue<pair<int, pair<int,int>>, vector<pair<int, pair<int,int>>>, greater<pair<int, pair<int,int>>> >> pq(size_Of_Cluster);
    // priority_queue<pair<int, pair<int,int>>, vector<pair<int, pair<int,int>>pi>, greater<pair<int, pair<int,int>>> > pq_root;
    ...
    for(i=0;;){
        ...
        int j;
        for(j=0;;){
            // if(!(j<elements_per_proc)&&((k+j)<key_freq.size())){
            //     break;
            // }
            ...
            // pq[i].push(make_pair(key_freq[k+j].first,make_pair(key_freq[k+j].second,i)));
    ...
    while(true){
        ...
        // if(!pq[pq_root.top().second.second].empty()){
        //     pq_root.push(pq[pq_root.top().second.second].top());
        //     pq[pq_root.top().second.second].pop();
        // }
...
void run_helper(int n, int size, int my_rank)
{
    ...
    // if(my_rank == 0)
    // {
    //     cout << endl << "Sorted Key Frequency" << endl;
    //     for(int i=0; i<n; i++)
    //     {
    //         cout << sorted_key_arr[i] << " -> " << sorted_freq_arr[i] << endl;
    //     }
    //     cout << endl;
    // }
    ...
    if(my_rank == 0)
    {
        ...
        //     cout << endl;
        // }
        // cout << "************* END OF Printing the root 2d array *************" << endl;
