...
int main(int argc, char *argv[])
{
    ...
    if (rank == root)
    {
        cin >> v[0] >> v[1];
        cin>> u[0] >>u[1];
        u[2] = ceil((double)v[1] / size);
    }
    MPI_Bcast(v, 2, MPI_INT, root, MPI_COMM_WORLD);
    // M = v[0];
    // N = v[1];
    // K = v[2];
    // T = v[3];
    ...
    M = v[0];
    N = v[1];
    MPI_Bcast(u,3, MPI_INT, root, MPI_COMM_WORLD);
    K = u[0];
    T = u[1];
    portion_length = u[2];
    // M = get_ele(v,0);
    ...
    for (int x = 0; x < T; x++)
    {
        ...
        if (rank != size - 1)
        {
            ...
            MPI_Send(&size_r, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
            for (int x = size_r; x > 0; x--)
            {
                Particle &p = list_crossed_upperlimit[x - 1];
                MPI_Send(p.serialize().data(), 4, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
            }
        }
        if (rank != 0)
        {
        ...
        if (rank != size - 1)
        {
            MPI_Recv(&size_r, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            int temp[4];
            for (int x = size_r; x > 0; x--)
            {
                MPI_Recv(temp, 4, MPI_INT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                my_particles.insert({temp[0], Particle(temp[0], temp[1], temp[2], temp[3])});
            }
        }
        if (rank != 0)
    ...
    size_r = part.size();
    MPI_Send(&size_r, 1, MPI_INT, root, 0, MPI_COMM_WORLD);
    for (int x = size_r; x > 0; x--)
    {
        Particle &p = part[x - 1];
    ...
    if (rank == root)
    {
        ...
        for (int rank = 0; rank < size; rank++)
        {
            MPI_Recv(&size_r, 1, MPI_INT, rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            int temp[4];
            for (int x = size_r; x > 0; x--)
            {
                MPI_Recv(temp, 4, MPI_INT, rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
